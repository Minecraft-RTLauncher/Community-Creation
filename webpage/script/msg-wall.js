document.addEventListener("alpine:init", () => {
    Alpine.data("messageBoard", () => ({
            ...
            if (!this.isLoading) {
                ...
                try {
                    const e = t ? 1 : this.currentPage,
                        // 使用组合后的API地址
                        a = await fetch(`${this.apiConfig.baseUrl}${this.apiConfig.endpoints.getMessages}?page=${e}`),
                        s = await a.json();
        ...
        async submitMessage() {
            ...
            this.isSubmitting = !0;
            try {
                const t = await fetch(`${this.apiConfig.baseUrl}${this.apiConfig.endpoints.sendMessage}`, {
                    method: "POST",
                    headers: {
        ...
        async sendLikeToBackend(t, e) {
            try {
                const a = await fetch(`${this.apiConfig.baseUrl}${this.apiConfig.endpoints.likeMessage}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json"
                    },
